...
def find_bbox(image, bboxes, prediction_model, decode_batch_predictions, f, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    num_classes = len(classes)
    image_h, image_w, _ = image.shape
    hsv_tuples = [(1.0 * x / num_classes, 1., 1.) for x in range(num_classes)]
    colors = list(map(lambda x: colorsys.hsv_to_rgb(*x), hsv_tuples))
    ...
    for i in range(num_boxes[0]):
        ...
        # print(box)
        xmin, ymin = (int(c1[0]), int(c1[1]))
        xmax, ymax = (int(c2[0]), int(c2[1]))
        # Convert array to image
        #img_crop = Image.fromarray(frame_save)  
        f.write('**************************\n')
        
        f.write('box: xmin = {}, ymin = {}, xmax = {}, ymax = {} \n'.format(xmin, ymin, xmax, ymax))          
        img_crop = np.copy(image[ymin:ymax, xmin:xmax, :])
        ...
        if show_label:
            ...
            bbox_mess += ' :' + text
            t_size = cv2.getTextSize(bbox_mess, 0, fontScale, thickness=bbox_thick // 2)[0]
            c3 = (counter * 100 + t_size[0], counter * 100 - t_size[1] - 3)
            cv2.rectangle(image, (counter * 100, counter * 100), (int(c3[0]), int(c3[1])), bbox_color, -1) #filled

            # cv2.putText(image, bbox_mess, (int(c1[0]), int(c1[1] - 2)), cv2.FONT_HERSHEY_SIMPLEX,
            #             fontScale, (0, 0, 0), bbox_thick // 2, lineType=cv2.LINE_AA)
            cv2.putText(image, bbox_mess, (counter * 100, counter * 100), cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale, (0, 0, 0), bbox_thick // 2, lineType=cv2.LINE_AA)
        bbxs.append([(int(c1[0]), int(c1[1])), (int(c2[0]), int(c2[1]))])

    return bbxs, detection_flg, image
